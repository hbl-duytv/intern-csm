...
func RegisterSuccess(c *gin.Context) {
	...
	if user, err := services.GetUserByToken(token); err == nil {
		...
		if user.Confirm == constant.DeactiveNumber {
			...
			if check, err := services.HasLimitTimeConfirm(user.ID); err != nil {
...
func SendConfirmRegister(c *gin.Context) {
	...
	if services.CreateAccount(username, password, email, "", "", "", constant.DeactiveNumber, constant.DeactiveNumber, constant.DeactiveNumber, token) == nil {
